...
public abstract class Transport<T extends Transport<T, C>, C extends TransportConfig> {
	...
	/**
	 * Set a new local address.
	 *
	 * @param localAddress a new local address
	 * @return a new {@link Transport} reference
	 */
	public final T localAddress(SocketAddress localAddress) {
		Objects.requireNonNull(localAddress, "localAddress");
		T dup = duplicate();
		dup.configuration().localAddress = localAddress;
		return dup;
	}
	...
	public final <O> T option(ChannelOption<O> key, @Nullable O value) {
		...
		if (ChannelOption.AUTO_READ == key) {
			log.error("ChannelOption.AUTO_READ is configured to be [false], it cannot be set to [{0}]", value);
			@SuppressWarnings("unchecked")
			T dup = (T) this;
			return dup;
		}
